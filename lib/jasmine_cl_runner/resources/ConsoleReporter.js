  ...
  function ConsoleReporter(options) {
    ...
    this.jasmineDone = function(options) {
      ...
      var specCounts = specCount + " " + plural("spec", specCount) + ", " +
        colored("green", passedCount + " passed") + ", ";

      if (failureCount > 0) {
        specCounts += colored("red", failureCount + " " + plural("failure", failureCount));
      } else {
        specCounts += failureCount + " " + plural("failure", failureCount);
      }

      if (pendingCount) {
        specCounts += ", " + colored("yellow", pendingCount + " pending " + plural("spec", pendingCount));
      }
